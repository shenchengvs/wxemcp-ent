...
//        var infobj='#tab1';
        ...
        function getListItem(vlist,vTart) {
            ...
            for(var i in vlist){
                listHtml+='<tr>'+'<td>'+
                ((vlist[i].point===null)?flag:vlist[i].point)+'</td><td>'
                +((vlist[i].startBm===null)?flag:vlist[i].startBm)+'</td><td>'
                +((vlist[i].endBm===null)?flag:vlist[i].endBm)+'</td><td>'
                +((vlist[i].multiplyingPower===null)?flag:vlist[i].multiplyingPower)+'</td><td>'
                +((vlist[i].data===null)?flag:vlist[i].data)+'</td></tr>';
            }
        ...
        $(mainDiv).infinite().on("infinite", function() {
            ...
            setTimeout(function() {
                if(isOver){//全部数据已经展示
                    ...
                    pindex++;
                    getPages( 'data/data_queryWaters.action',
                        {
                    	  accountKey:vKey,
