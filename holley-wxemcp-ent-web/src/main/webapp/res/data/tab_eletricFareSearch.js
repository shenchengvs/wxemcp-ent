...
//        FastClick.attach(document.body);
        ...
        function getListItem(vlist,vTart) {
            var listHtml='';
            for(var i in vlist){
            	var oDay=((vlist[i].usableDay===null)?flag:vlist[i].usableDay);
            	oDay=(oDay<0)?flag:oDay;
              ...
              '</label><span class="weui-form-preview__value">预计可用:'+'<em class="text-primary margin-l5">'+(oDay)+'天</em></span></div><div class="weui-form-preview__item"><label class="weui-form-preview__label">结算时间:'+'<em class="margin-l5">'+
              ((vlist[i].billTime===null)?flag:vlist[i].billTime)+'</em></label></div><div class="weui-form-preview__item">'+'<label class="weui-form-preview__label">余额:'+'<em class="text-green margin-l5">'+((vlist[i].usableMoney===null)?flag:vlist[i].usableMoney)+'</em></label>'+'<span class="weui-form-preview__value">实际电费:'+'<em class="text-red margin-l5 font-16">'+((vlist[i].fee===null)?flag:vlist[i].fee)+'</em></span>'+'</div></div></a>';
            }
            
            $(vTart).append(listHtml);
            $('.more-content-item').on('click',function(e){
            	var obj=$(this);
            	storage.setItem('customerid',obj.attr('role'));//缓存用户id
            	storage.setItem('month',$(timeObj).val().trim());//缓存当前日期
            	storage.setItem('hm',obj.attr('name'));//缓存户名
            
                window.location.href="data/initPowerFeeDetailPage.action";
            })
        ...
        function initPage(oWord,isFresh){ //默认更新
            ...
            getPages(
                'data/data_queryPowerFees.action',
                {
                    accountKey:vKey,
                    keyWord:oWord,//搜索关键字
                    pageIndex:pindex,
        ...
        $(mainDiv).infinite().on("infinite", function() {
            ...
            setTimeout(function() {
                if(isOver){//全部数据已经展示
                    ...
                    pindex++;
                    getPages(  'data/data_queryPowerFees.action',
                            {
                        accountKey:vKey,
