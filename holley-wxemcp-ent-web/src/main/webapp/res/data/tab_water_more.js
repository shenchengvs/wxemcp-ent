...
//        
            ...
            for(var i in vlist){
                ...
                if(rule.type===1){
                	...
                	+((vlist[i].fee===null)?flag:(vlist[i].fee))+'</td></tr></tbody></table></div></div>';
                		...
                		for(var k = 0; k < 5; k++) { // 最多可能有5个水价
                			if(k == 4 || priceList[k + 1] == null) { // 已经是最后一条，或者本条的下一条不存在，意为本条套用结尾格式
                				listHtml += '<p>第'+chineseNum[k]+'阶梯水价'+priceList[k]+'元/度, 已用'+
                				...
                				listHtml += '<p>第'+chineseNum[k]+'阶梯水价'+priceList[k]+'元/度, 基数'+(energyList[k + 1] - energyList[k])+', 已用'+used+'</p>';
                			}
                		}
        ...
        function initPage(isFresh){ //默认更新
            ...
            $('.index-title').text(storage.getItem('watermonth'));//日期
	    	$('.account-name').text(storage.getItem('waterhm'));//户名
	    	$('.to-tel').attr('href','');
	    	    
	    	$('.account-id').text('');//户号
	    	$('.user-phone').text('');//水话号码
	    	$('.linkman').text('');//联系人
            ...
            getPages(
          	      ...
          	      {
          	          accountKey:vKey,
          	          customerid:storage.getItem('watercustomerid'),
          	          month:storage.getItem('watermonth')//月份
//          	          month:'2017-12'
                  },function (res) {
                    ...
                    var olist=res.data.datas;
       	    	    ...
       	    	    $('.to-tel').attr('href','tel:'+((rest.phone===null)?flag:rest.phone)+'#mp.weixin.qq.com');
       	    	    ...
       	    	    t1+='<tr class="th">'+'<td>计算月份</td>'+
                    '<td>水表数量</td>'+
                    '<td>联系人</td>'+
                    '<td>手机号码</td></tr>'
       	    	    +'<tr><td>'+((rest.billTime===null)?flag:rest.billTime)+'</td><td>'
       	    	    +((olist.length===0)?'0':olist.length)+'</td><td>'
       	    	    +((rest.linkman===null)?flag:rest.linkman)+'</td><td><img class="table-icon" src="res/images/tel_phone_icon.png" />'
       	    	    +((rest.phone===null)?flag:rest.phone)+'</td></tr>'+'<tr class="th"><td>消耗水量</td>'+
                    '<td>追退水费</td>'+'<td>应付水费</td>'+'<td>账户余额</td></tr>'
                    ...
                    +'<tr class="th">'+'<td>总余额</td>'+
                    '<td colspan="2">冻结余额</td>'+
                    '<td>可用余额</td></tr>'
                    +'<tr><td>'+((rest.total===null)?flag:rest.total)+'</td><td colspan="2">'
...
//       	    	  
//       	    	    var t2='';//表2
//       	    	    t2+='<tr><td>'+((rest.used===null)?flag:rest.used)+'</td><td>'+((rest.ztFee===null)?flag:rest.ztFee)+'</td><td>'+((rest.fee===null)?flag:rest.fee)+'</td><td>'+((rest.usableMoney===null)?flag:rest.usableMoney)+'</td></tr>';
//    	    	    $('.table2').html(t2);
//    	    	    
//    	    	    var t3='';//表3
//       	    	    t3+='<tr><td>'+((rest.total===null)?flag:rest.total)+'</td><td>'+((rest.freeze===null)?flag:rest.freeze)+'</td><td>'+((rest.usable===null)?flag:rest.usable)+'</td></tr>';
//    	    	    $('.table3').html(t3);
