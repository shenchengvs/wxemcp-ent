...
//      $.showLoading();
        function getListItem(vlist,vTart) {
            ...
            for(var i in vlist){
                ...
                ((vlist[i].point===null)?flag:vlist[i].point)+'<span class="weui-media-box__title-after text-red font-16">'+((vlist[i].data===null)?flag:vlist[i].data)+'m<sup>3</sup></span></h4><p class="weui-media-box__desc text-green">'+((vlist[i].ownername===null)?flag:vlist[i].ownername)+'<span class="weui-media-box__title-after text-light-gray">'+((vlist[i].dataTime===null)?flag:vlist[i].dataTime)+'</span></p></div></a>';
            }
        ...
        function initPage(oWord,isFresh){ //默认更新
            ...
            getPages(
                'data/data_queryRealWaters.action',
                {
                    accountKey:vKey,
                    keyWord:oWord,//搜索关键字
                    pageIndex:pindex,
        ...
        $(infobj).infinite().on("infinite", function() {
            ...
            setTimeout(function() {
                if(isOver){//全部数据已经展示
                    ...
                    pindex++;
                    getPages( 'data/data_queryRealWaters.action',
                        {
                    	  accountKey:vKey,
