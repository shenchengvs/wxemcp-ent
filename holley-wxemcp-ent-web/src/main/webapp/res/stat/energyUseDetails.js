...
//	                    	list.reverse();
       ...
       function getListItem(vlist,vTart) {
           ...
           for(var i in vlist){
               ...
               '<div class="text-init font-18 init-item-title">当'+(monthtype>1?'月':'日')+'消耗电能的总量<span class="text-primary init-money">'
               +(vlist[i].nowsum===null?noFlag:vlist[i].nowsum)+'</span>kWh</div><div class="weui-flex">'+
               '<div class="weui-flex__item init-item-sub"><p>同上'+(monthtype>1?'月':'日')+'增长<span class="text-red init-upval1">'+(vlist[i].upratethanla===(null||0)?noFlag:vlist[i].upratethanla)+'</span>%</p></div><div class="weui-flex__item init-item-sub text-right"><p>'+
               (monthtype>1?'同去年本月':'同上周本日')+
        ...
        function initPage(tartId,isFresh){ //默认更新
            ...
            getPages(
                'stat/stat_queryEnergyUseDetails.action',
                {
                    accountKey:vKey,
//                    datatype:2,//数据类型
//                    datetime:'2017-09',//日期
