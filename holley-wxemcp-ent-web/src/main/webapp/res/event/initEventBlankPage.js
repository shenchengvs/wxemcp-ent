...
//                    console.log(values);
        ...
        getDatePlugins(getNowDate($('#exterMonth1')),'#exterMonth1','日期选择',function(vTime){
        	if(oldTime1!=vTime){
        	   initPage(listDiv1,infDiv1,1,false,true);//初始化tab1
        	   oldTime1=vTime;
        	}
        });
        getDatePlugins(getNowDate($('#exterMonth2')),'#exterMonth2','日期选择',function(vTime){
        	if(oldTime2!=vTime){
        	   initPage(listDiv2,infDiv2,2,false,true);//初始化tab2
        	   oldTime2=vTime;
        	}
        });
        getDatePlugins(getNowDate($('#exterMonth3')),'#exterMonth3','日期选择',function(vTime){
        	if(oldTime3!=vTime){
        	   initPage(listDiv3,infDiv3,3,false,true);//初始化tab3
        	   oldTime3=vTime;
        	}
        });
        ...
        $(infDiv3).find('.weui-pop-div').val('全部').data({"values":0});
            ...
            $("#sendType").select({
                ...
                onClose: function (vals) {
                   ...
                   if(oldSelect3!=oVal){
                	   initPage(listDiv3,infDiv3,3,false,true);//初始化tab1
                	   oldSelect3=oVal;
                   }
                },
                onOpen: function () {
         ...
         function getListItem2(vlist,vTart) {
             ...
             for(var i in vlist){
                  ...
                  var addClass=vlist[i].money>0?'text-green':'text-red';
                  var valMoney=(vlist[i].money===null)?0:vlist[i].money;
                  valMoney=vlist[i].money>0?'+':'';
                 ...
                 '<h4 class="weui-media-box__title">'+((vlist[i].paywayDesc===null)?flag:vlist[i].paywayDesc)+'<span class="weui-media-box__title-after '+addClass+' font-16">'+valMoney+''+
                     ((vlist[i].money===null)?flag:vlist[i].money)+'</span></h4><p class="weui-media-box__desc">'+((vlist[i].hm===null)?flag:vlist[i].hm)+'<span class="weui-media-box__title-after text-light-gray">'+finalTime+'</span></p>'+'</div></a>';
             }
             $(vTart).append(listHtml);
         }
         ...
         function getListItem3(vlist,vTart) {
             ...
             for(var i in vlist){
                  ...
                  var finalTime=xtime.substring(0,xtime.indexOf(' '));
                  listHtml+=' <a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg wui-cell-nopadding-top">'+'<div class="weui-media-box__hd weui-media-box-line"></div>'+'<div class="weui-media-box__bd">'+
                 '<h4 class="weui-media-box__title">'+((vlist[i].hm===null)?flag:vlist[i].hm)+'<span class="weui-media-box__title-after text-primary font-16">'+((vlist[i].sendTypeDesc===null)?flag:vlist[i].sendTypeDesc)
       +'</span></h4> <p class="weui-media-box__desc text-green">'+((vlist[i].phone===null)?flag:vlist[i].phone)+'<span class="weui-media-box__title-after text-light-gray">'+finalTime+'</span></p>'+'</div></a>';
         ...
         function initPage(listobj,infobj,tabId,isFresh,notAll){ //listobj 加载列表的位置； infobj 滚动加载更多；vindex 当前页码数；tabId 当前tab值；vloading 滚动加载更多；vrefresh 刷新 
             ...
             if(tabId===1){//tab1拉合闸记录
            	 ...
            	 if(!notAll){
//            		 
//            		 getNowDate($(infobj).find('.mui-date'));
//                	 $(infobj).find('.weui-pop-div').val('全部').data({"values":0});
...
//                     console.log(selectVal+' '+thisTime);
        ...
        initPage(listDiv1,infDiv1,1,false,false);//初始化tab1
        ...
        $('.weui-navbar__item').on("click",function(){//点击不同tab所触发的事件
        	...
        	if(vrole==='1'){
        		...
        		if(vrole==='2'){
        			initPage(listDiv2,infDiv2,2,false,false);//初始化tab2
        		}else{
        			initPage(listDiv3,infDiv3,3,false,false);//初始化tab3
        		}
        ...
        // pullToRefresh
        //#tab1
        $(infDiv1).pullToRefresh().on("pull-to-refresh", function() {
         	 var self=this;
             if(toRefresh1) return;
             toRefresh1 = true;
             setTimeout(function() {
                 initPage(listDiv1,self,1,true,false);//初始化tab1
             }, 1000);
         });
        //#tab2
        $(infDiv2).pullToRefresh({
        	distance:0
             ...
             toRefresh2 = true;
             setTimeout(function() {
                 initPage(listDiv2,self,2,true,false);//初始化tab2
             }, 1000);
         });
        
      //#tab3
        $(infDiv3).pullToRefresh().on("pull-to-refresh", function() {
         	 var self=this;
             if(toRefresh3) return;
             toRefresh3 = true;
             setTimeout(function() {
                 initPage(listDiv3,self,3,true,false);//初始化tab3
             }, 1000);
         });
        ...
        //tab1
         ...
         $(infDiv2).infinite().on("infinite", function() {
             ...
             if(loading2) return;
             loading2 = true;
             console.log('infinite222');
             var ohtl='<div class="weui-loadmore btm-loadmore">' +
             '<i class="weui-loading"></i>' +
             '<span class="weui-loadmore__tips">正在加载</span>' +
             '</div>';
             var div=$(this).find(listDiv2);
             div.append(ohtl);
